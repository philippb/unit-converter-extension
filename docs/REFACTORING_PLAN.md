...
test('does not convert currency followed by "in"', () => {
    ...
    expect(convertText('Saved $2,400 in the first year alone')).toBe(
        'Saved $2,400 in the first year alone'
    );
});

test('does not convert "in" after numbers in text', () => {
    expect(convertText('roughly 32% of customers in California')).toBe(
        'roughly 32% of customers in California'
    );
});
```

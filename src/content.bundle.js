(() => {
    ...
    var require_regex = __commonJS({
        'src/parsing/regex.js'(exports, module) {
            ...
            function createRegexFromTemplate(unitBig, unitSmall = '') {
                ...
                const combined =
                    unitSmall && unitSmall.length > 0 ? `${unitBig}|${unitSmall}` : unitBig;
                ...
                try {
                    ...
                    if (parts.length >= 2) {
                        ...
                        if (step1 && step1 > 0) {
                            ...
                            else if (reSecondary.test(unit1))
                                steps.push(step1 * LENGTH_INCH_TO_METERS);
                        }
                    }
                    if (parts.length >= 4) {
                        const val2 = parts[2];
                        ...
                        if (step2 && step2 > 0) {
                            ...
                            else if (rePrimary.test(unit2))
                                steps.push(step2 * LENGTH_FOOT_TO_METERS);
                        }
                    }
    ...
    var require_ranges = __commonJS({
        'src/formatting/ranges.js'(exports, module) {
            ...
            function formatWeightRange(g1, g2) {
                ...
                if (a >= 1e3 && b >= 1e3)
                    return `${formatNum(a / 1e3)}\u2013${formatNum(b / 1e3)} kg`;
    ...
    var require_patterns = __commonJS({
        'src/exclusions/patterns.js'(exports, module) {
            ...
            var STANDALONE_IN_PATTERN = new RegExp('^\\s*\\d[\\d\\s,./]*\\s*in\\.?\\s*$', 'i');
            var QUOTE_PAIRS = [
                { open: '"', close: '"' },
                { open: "'", close: "'" },
                { open: '\u201C', close: '\u201D' },
    ...
    var require_length = __commonJS({
        'src/units/length.js'(exports, module) {
            ...
            var { inferResolutionMetersFromNumber, inferResolutionMetersFromLengthMatch } =
                require_precision();
            var { formatLengthMeasurement } = require_units2();
            var { shouldExcludeMatch } = require_patterns();
            var DOUBLE_APOSTROPHE_TOKENS = ["''", '\u2019\u2019'];
            ...
            function convertLengthText(text) {
                ...
                if (
                    ...
                    converted = converted.replace(feetInchesRegex, function () {
                        ...
                        const meters = convertLengthToMeters(
                            parsed.primary.value,
                            parsed.secondary.value
                        );
            ...
            function convertWeightText(text) {
                ...
                if (
                    ...
                    converted = converted.replace(weightRegex, function () {
                        ...
                        const grams = convertWeightToGrams(
                            parsed.primary.value,
                            parsed.secondary.value
                        );
            ...
            function convertLiquidText(text) {
                ...
                if (lower.includes('cup') || lower.includes('fl oz') || lower.includes('fluid')) {
                    ...
                    converted = converted.replace(cupsFluidOuncesRegex, function () {
                        ...
                        const liters =
                            parsed.primary.value * LIQUID_CUP_TO_L +
                            parsed.secondary.value * LIQUID_FLOZ_TO_L;
                        if (liters === 0) return match;
                        const resolutionLiters = inferResolutionFromParsedMeasurement(parsed, {
                ...
                if (
                    ...
                    converted = converted.replace(tablespoonsTeaspoonsRegex, function () {
                        ...
                        const liters =
                            parsed.primary.value * LIQUID_TBSP_TO_L +
                            parsed.secondary.value * LIQUID_TSP_TO_L;
                        if (liters === 0) return match;
                        const resolutionLiters = inferResolutionFromParsedMeasurement(parsed, {
        ...
        'src/units/area.js'(exports, module) {
            ...
            function convertAreaText(text) {
                ...
                if (
                    ...
                    lower.includes(' ft2') ||
                    lower.includes('ft^2') ||
                    lower.includes('ft\xB2') ||
                    lower.includes(' in2') ||
                    lower.includes('in^2') ||
                    lower.includes('in\xB2') ||
                    lower.includes(' yd2') ||
                    lower.includes('yd^2') ||
                    lower.includes('yd\xB2') ||
                    lower.includes(' mi2') ||
                    lower.includes('mi^2') ||
                    lower.includes('mi\xB2') ||
                    lower.includes('acre')
                ) {
                    ...
                    for (const { units, factor } of groups) {
                        const areaRegex = new RegExp(
                            String.raw`\b(${VALUE_PART})\s*${units}(?!\s*\()`,
                            'giu'
                        );
    ...
    // src/units/awg.js
    var require_awg = __commonJS({
        'src/units/awg.js'(exports, module) {
            var { LENGTH_INCH_TO_METERS } = require_constants();
            var { formatLengthMeasurement } = require_units2();
            var { shouldExcludeMatch } = require_patterns();
            var AWG_REGEX = /(\d{1,3}(?:\.\d+)?)(?:\s*-\s*)?\s*AWG\b/gi;
            var AWG_RESOLUTION_METERS = 1e-6;
            function convertAwgToMeters(gaugeValue) {
                const gauge = Number(gaugeValue);
                if (!Number.isFinite(gauge)) return NaN;
                const diameterInInches = 5e-3 * Math.pow(92, (36 - gauge) / 39);
    ...
    var require_converter = __commonJS({
        'src/converter.js'(exports, module) {
            ...
            var { convertAwgText } = require_awg();
            var {
                LIQUID_GALLON_TO_L,
                LIQUID_QUART_TO_L,
                LIQUID_PINT_TO_L,
            ...
            function hasRelevantUnits(text) {
                ...
                const RE_INCH_SYMBOL_HINT = new RegExp(
                    String.raw`(?:${numToken})\s*[${INCH_SYMBOLS}]`,
                    'u'
                );
            ...
            function convertText(text) {
                ...
                function mergeUnitRanges(s, config) {
                    ...
                    for (let i = 0; i < matches.length; i++) {
                        ...
                        if (i > 0) {
                            ...
                            if (RANGE_SEP_RE.test(between)) {
                                ...
                                const token = addPlaceholder(
                                    `${s.slice(prev.start, curr.end)} (${formatted})`
                                );
                        ...
                        if (tail) {
                            ...
                            const isSingleUnit =
                                (hasPrimary && !hasSecondary) || (!hasPrimary && hasSecondary);
                            if (isSingleUnit) {
                ...
                const lengthConverters = {
                    feetInches: (parsed) =>
                        convertLengthToMeters(parsed.primary.value, parsed.secondary.value),
    ...
    var require_content = __commonJS({
        'src/content.js'(exports) {
            ...
            var { getUnitRegexes, extractFirstValueToken, parseMeasurementMatch } =
                require_measurement();
            var {
                formatLengthMeasurement,
                formatWeightMeasurement,
            ...
            var { convertAreaText } = require_area();
            var { convertTemperatureText } = require_temperature();
            var { convertTimeZone, convertTimeZoneText, TARGET_TIMEZONE, TARGET_TIMEZONE_OFFSET } =
                require_timezone();
                            ...
                            while (j < newText.length) {
                                ...
                                if (
                                    ...
                                    while (isWhitespace(newText[j]) && newText[j + 1] === '(') {
                                        if (frag && doc)
                                            frag.appendChild(doc.createTextNode(newText[j]));
                                        j += 1;
                                    }
                                    if (newText[j] !== '(') {
                                        buffer += newText[j];
